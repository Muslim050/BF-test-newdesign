...
const AdvertiserTableData = ({advertisers}) => {
  ...
  const fetchCpm = useCallback(async (id) => {
    try {
      ...
      await axios.get(
        `${backendURL}/order/cpm/?advertiser=${id}`,
        {
          headers: {
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        {open && <EditAdvModal onClose={handleClose} currentAdvertiser={currentAdv} fetchCpm={fetchCpm} />}
      </Dialog>
      {/*Редактирование*/}
        ...
        {advertisers.map((person, i) => {
          return (
            <TableRow key={person.id} className={style.table__tr}>
              ...
              <TableCell data-label="Наименование Компании"  className={`font-normal text-${textColor} text-sm`}>{person.name}</TableCell>
              {hasRole('admin') && (
                <>
                  <TableCell data-label="CPM_Preroll"  className={`font-normal text-${textColor} text-sm`}><FormatterView data={person.cpm_preroll} /></TableCell>
                  <TableCell data-label="CPM_Mixroll"  className={`font-normal text-${textColor} text-sm`}><FormatterView data={person.cpm_mixroll} /></TableCell>
                </>
              )}

              {hasRole('admin') && (
                <>
                  <TableCell data-label="Target_Preroll"  className={`font-normal text-${textColor} text-sm`}>
                  ...
                  <TableCell data-label="Target_Mixroll"  className={`font-normal text-${textColor} text-sm`}>
              ...
              <TableCell data-label="Email"  className={`font-normal text-${textColor} text-sm`}>{person.email}</TableCell>
              <TableCell data-label="Номер телефона"  className={`font-normal text-${textColor} text-sm`}><FormatterPhone phoneNumber={person.phone_number} /></TableCell>
              <TableCell data-label="Рекламное агенство"  className={`font-normal text-${textColor} text-sm`}>
