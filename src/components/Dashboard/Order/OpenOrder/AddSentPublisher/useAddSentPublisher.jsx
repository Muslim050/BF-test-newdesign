...
import PopoverEditView from "@/components/Dashboard/Order/OpenOrder/PopoverEditView.jsx";
...
export const useAddSentPublisher = ( expandedRows, onceOrder) => {
  ...
  const listsentPublisherWithIndex = listsentPublisher?.results?.map((item, index) => ({
    ...item,
    rowIndex: index, // Добавляем rowIndex
  }));
  const forceUpdate = React.useReducer(() => ({}))[1];
  const handlePopoverClick = (rowIndex) => {
    setOpenPopoverIndex((prevIndex) => (prevIndex === rowIndex ? null : rowIndex));
  };
  ...
  const columns = React.useMemo(
    () => [
      ...
      {
        accessorFn: (row) => row.age_range, // Преобразование в число
        id: 'Возраст',
        cell: (info) => info.getValue(),
        filterFn: 'includesStringSensitive', //note: normal non-fuzzy filter column - case sensitive
        header: () => <span>Возраст</span>,
      },
      {
      ...
      },
      {
        ...
        cell: ({row}) => <div className='flex gap-2'>
          {row.original.is_sent_to_publisher ? null : (
            <button
              onClick={() => setCurrentOrder((prev) => (prev?.id === row.original.id ? null : row.original))}
