...
import {TableRow, TableHeader, Table, TableHead, TableBody} from 'src/components/ui/table';
...
////////////////////////////
  ...
  const uniqueChannelName = new Set(publisherReport.map((item) => item.channel_name));
  ...
  const handleSearch = () => {
    if (selectedChannel || selectedPublisher) {
      ...
      const formattedStartDateMonth = startDateMonth ? format(startDateMonth, 'yyyy-MM-dd') : undefined;
      const formattedEndDateMonth = endDateMonth ? format(endDateMonth, 'yyyy-MM-dd') : undefined;
  ...
  React.useEffect(() => {
    if (handleSearch) {
      const totalBudgetD = publisherReport.reduce((total, person) => total + (person.budget_fact || 0), 0);
      const totalViewsD = publisherReport.reduce((total, person) => total + (person.recorded_view_count || 0), 0);
      const totalComisyD = publisherReport.reduce((total, person) => total + (person.agency_commission_total || 0), 0);
      const totalComisyAdtechD = publisherReport.reduce((total, person) => total + (person.adtechmedia_commission_total || 0), 0);
      const totalbudjetChannelD = publisherReport.reduce((total, person) => total + (person.channel_budget_total || 0), 0);
  ...
  const headers = [
    ...
    { key: 'commission_rate', label: `Бюджет - ${uniqueChannelNameFiltered[0]}` },
  ];
      ...
      {status === 'loading' ? (
        ...
        <div className="tableWrapper">
            ...
            <div className='flex items-center justify-end w-full'>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="ghost"
                    className="my-4 bg-brandPrimary-1 rounded-lg hover:bg-brandPrimary-50 text-white no-underline hover:text-white h-[48px]"
                  >
          ...
          {publisherReport && publisherReport.length ? (
            <div className={`border_container glass-background w-full rounded-xl my-4`}>
              <div className='border_design'>
                <div className="flex justify-between items-center gap-2">
                  ...
                  <div className="rounded-2xl">
                    <InfoCardsTop uniqueChannelNameFiltered={uniqueChannelNameFiltered}
                                  totalbudjetChannel={totalbudjetChannel} totalViews={totalViews}
                                  totalBudget={totalBudget} totalComisy={totalComisy}
