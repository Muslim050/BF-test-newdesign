...
export default function LinkedVideo({ selectedId, onClose }) {
  ...
  console.log('LinkedVideo', selectedId)
  const {
    register,
  ...
  const onSubmit = async (data) => {
    try {
      const videoLinkResponse = await dispatch(inventoryPublish({ data })).unwrap()
    ...
    <>
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div>
            <div>
              <Label className="text-sm	text-white pb-2 flex gap-0.5">Ссылка на Видео<span className='text-red-500 ml-0.5'>*</span>
