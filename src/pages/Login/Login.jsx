...
//         <div
//           className="w-full lg:grid h-screen lg:grid-cols-2  bg-[#090E35] p-6 ">
//           <div
//             ref={leftRef}
...
//                 <div  ref={mainTitleRef} className='font-normal	text-6xl	text-white 2xl:text-6xl xl:text-5xl  lg:text-4xl'>
//                   Увеличьте охваты вашего бренда с помощью брендированной рекламы
//                 </div>
//                 <div  ref={mainSubtitleRef} className='text-2xl	font-normal	text-white'>Brandformance - это онлайн видео площадка созданная для увеличения показателей вашего бренда через
//                   видео контент.
//                 </div>
//               </div>
...
//                       <CarouselItem key={index} className="pl-1 mr-1 md:basis-80 h-[210px]"             ref={(el) => (carouselItemsRef.current[index] = el)}
//                       >
//                         <div className="">
//                           <Card>
//                             <CardContent
//                               className="flex aspect-square items-center justify-center p-6 h-[210px] w-[308px]">
...
import { login } from 'src/redux/auth/authSlice.js'; // Проверьте правильность пути
import { toast } from 'react-hot-toast';
import { gsap } from 'gsap';
import { Button } from 'src/components/ui/button';
import { Input } from 'src/components/ui/input';
...
<<<<<<< HEAD
  ...
  const mainSubtitleRef = React.useRef(null);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="w-full lg:grid h-screen lg:grid-cols-2 bg-[#090E35] p-6">
        <div ref={leftRef} className="hidden bg-[#090E35] lg:block border_container_login glass-background rounded-[22px]">
          <div className="flex flex-col justify-around h-full">
            <div className="xl:px-[110px] lg:px-[50px] flex flex-col gap-6">
              <div ref={mainTitleRef} className="font-normal text-6xl text-white 2xl:text-6xl xl:text-5xl lg:text-4xl">
                Увеличьте охваты вашего бренда с помощью брендированной рекламы
              </div>
              <div ref={mainSubtitleRef} className="text-2xl font-normal text-white">
                Brandformance - это онлайн видео площадка созданная для увеличения показателей вашего бренда через видео контент.
              </div>
            </div>
            <div className="pl-[110px] flex gap-2 w-full relative">
              <Carousel className="w-full" opts={{ loop: true }}>
        ...
        <div ref={sectionRef} className="flex items-center justify-center py-12">
          <div className="mx-auto grid w-[450px] gap-6">
            <div ref={titleRef} className="grid gap-2 text-center">
              <h1 className="text-[32px] font-bold text-white">Brandformance</h1>
              <p className="text-balance text-muted-foreground">Let’s sign you in</p>
            </div>
            <div ref={firstRef} className="relative">
              <Input
                id="login"
                type="text"
                {...register('login', { required: 'Поле обезательно к заполнению' })}
                className={`border-[1px] rounded-[20px] h-[73px] p-[26px] text-white bg-[#0A0F3633] text-base ${
                  errors.login ? 'border-red-500' : 'border-[#123057]'
                }`}
                placeholder="Логин"
                ...
                autoComplete="off"
              />
              <span className="text-red-500 text-sm">{errors?.login && <p>{errors?.login?.message}</p>}</span>
            </div>
            <div ref={secondRef} className="relative">
              <Input
                id="password"
                type={showPasswordOld ? 'text' : 'password'}
                {...register('password', { required: 'Поле обезательно к заполнению' })}
                className={`border-[1px] rounded-[20px] h-[73px] p-[26px] text-white bg-[#0A0F3633] text-base ${
                  errors.password ? 'border-red-500' : 'border-[#123057]'
                }`}
                placeholder="Пароль"
              ...
              <div onClick={handleTogglePasswordOld} className="absolute top-[35%] right-[26px] cursor-pointer">
                {showPasswordOld ? <Eye className="text-white" /> : <EyeOff className="text-white" />}
              </div>
            </div>
            <div ref={buttonRef}>
              <Button
                className={`${isValid ? 'bg-[#2A85FF] hover:bg-[#2A85FF99]' : 'bg-[#616161]'} w-full rounded-[20px] h-[64px] text-white`}
                disabled={!isValid || isLogin}
              >
                Войти
                {isLogin && <Loader2 className="ml-2 h-6 w-6 animate-spin" />}
